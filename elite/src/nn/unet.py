...
from src.nn.blocks import weights_initializer
from src.nn.layers import Conv2dWNUB, ConvTranspose2dWNUB, glorot
...
class UNetConcat(nn.Module):
    def __init__(
        ...
        self.down1 = nn.Sequential(
            nn.Conv2d(in_channels, F, 4, 2, 1),
            nn.LeakyReLU(0.2),
        )
        self.down2 = nn.Sequential(
            nn.Conv2d(F, 2 * F, 4, 2, 1),
            nn.LeakyReLU(0.2),
        )
        self.down3 = nn.Sequential(
            nn.Conv2d(2 * F, 4 * F, 4, 2, 1),
            nn.LeakyReLU(0.2),
        )
        self.down4 = nn.Sequential(
            nn.Conv2d(4 * F, 8 * F, 4, 2, 1),
            nn.LeakyReLU(0.2),
        )
        self.down5 = nn.Sequential(
            nn.Conv2d(8 * F, 16 * F, 4, 2, 1),
            nn.LeakyReLU(0.2),
        )
        self.down6 = nn.Sequential(
            nn.Conv2d(16 * F, 32 * F, 4, 2, 1),
            nn.LeakyReLU(0.2),
        )
        self.up0 = nn.Sequential(
            nn.ConvTranspose2d(
        ...
        self.up5 = nn.Sequential(
            nn.ConvTranspose2d(2 * F, F, 4, 2, 1),
            nn.LeakyReLU(0.2),
        )
        self.out = nn.Conv2d(
            F + in_channels, out_channels, kernel_size=1
        )
        # self.apply(lambda x: la.glorot(x, 0.2))
        # la.glorot(self.out, 1.0)
